...
def eval():
    ...
    with torch.no_grad():
        for batch in tqdm(eval_loader, desc="Evaluating"):
            ...
            # 7) Compute the loss
            Models.sketch_arguments.face_aggregate.vis_planar(gt_matrix, planes, node_features)
            print("output", output)
            probabilities = torch.sigmoid(output)
            print("probabilities", probabilities)
            Models.sketch_arguments.face_aggregate.vis_planar(probabilities, planes, node_features)
